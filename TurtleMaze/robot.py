...
class Robot(object):
    ...
    def pf_test(self):
        attractive = Point((1, 1), 0.01, 2.0, 'ATTRACTIVE')
        repulsive = Point((5.5, 5.5), 0.20, 0.1, 'REPULSIVE')
    ...
    def solve_maze1(self):
        ...
        while not completed:
            ...
            #angle = int(math.atan2(ty - ry, tx - rx))
            #radius = int(dist * 0.5 / math.sin(rt - angle)) if rt - angle != 0.0 else int('Inf')
            #print (angle)
            data = 196
            distance = self.laser_data[data]
            #print '\n\n\n'
